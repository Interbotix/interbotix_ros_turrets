...
def main():
    ...
    robot = InterbotixTurretXS(robot_model="pxxls", pan_profile_type="velocity", tilt_profile_type="velocity")

    # move the pan servo to 1.0 radians; make the max velocity about pi rad/s (equivelant to 131), and the max acceleration about 5.6 rad/s^2 (equivelant to 15)
    # then arbitrarily wait 1 second until the motion finishes to return control to the user
    robot.turret.pan(position=1.0, profile_velocity=131, profile_acceleration=15, delay=1.0)

    # move the tilt servo to 1.5 radians; make the max velocity about pi/2 rad/s and the max acceleration about 1.87 rad/s^2;
    # then arbitrarily wait 2 seconds to return control to the user
    robot.turret.tilt(position=1.5, profile_velocity=65, profile_acceleration=5, delay=2.0)
    ...
    robot.turret.pan_tilt_move(-0.8, -1, pan_profile_velocity=98, pan_profile_acceleration=5, tilt_profile_velocity=33, tilt_profile_acceleration=0, delay=3.0)
