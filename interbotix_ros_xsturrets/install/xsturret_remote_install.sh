...
if [ $ubuntu_version == "16.04" ]; then
  ...
  echo -e "Interbotix Remote Turret only works with 16.04, 18.04, or 20.04"
  exit 1
fi

# Step 1: Install Crawler packages
INTERBOTIX_WS=~/interbotix_ws
if [ ! -d "$INTERBOTIX_WS/src" ]; then
  echo "Installing Simulation/Visualization ROS packages for the Interbotix Turrets..."
...
echo -e "NOTE: Remember to comment out the ${ORANGE}source $INTERBOTIX_WS/devel/setup.bash${NC} and ${ORANGE}export ROS_MASTER_URI=http://$HOSTNAME.local:11311${NC} lines from the ~/.bashrc file when done using the Turret! Then close out of your terminal and open a new one."
